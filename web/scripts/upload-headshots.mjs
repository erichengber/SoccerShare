...
const headshotsDir = path.join(projectRoot, "public", "headshots");
...
async function main() {
  ...
  const assetNames = (await fs.readdir(headshotsDir)).sort();
  const uploadedAssets = [];

  for (const assetName of assetNames) {
    const localPath = path.join(headshotsDir, assetName);
    const fileStats = await fs.stat(localPath);
    if (!fileStats.isFile()) continue;

    const fileBuffer = await fs.readFile(localPath);
    const storagePath = `seed/public/headshots/${assetName}`;
    await uploadAsset({
      supabaseUrl,
      supabaseKey,
      bucket,
